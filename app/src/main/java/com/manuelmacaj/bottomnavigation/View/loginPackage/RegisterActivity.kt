...
class RegisterActivity : AppCompatActivity() {
    ...
    private val mRegister = FirebaseAuth.getInstance() //istanza firebase riferita alla sezione di autenticazione
    ...
    private fun registerNewAccount(nomeCognome: String, email: String, password: String) {
        mRegister.createUserWithEmailAndPassword(email, password).addOnCompleteListener(this) {task -> //proviamo a registrare utente
    ...
    private fun isValidPassword(pwd: String): Boolean {
        return if ((!pwd.isEmpty()) && (pwd.length >= 4))
            true
        else false
    }
